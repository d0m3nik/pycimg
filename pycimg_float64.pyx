cdef class CImg_float64:
    cdef CImg[float64] _cimg;

    # Constructors
#    def __cinit__(self, filename):
#        byte_string = filename.encode('UTF-8')
#        cdef char* fn = byte_string
#        self._cimg = CImg[float64](fn)
    ...
    def load_cimg_float16(self, filename):
        byte_string = filename.encode('UTF-8')
        cdef char* fn = byte_string
        self._cimg = from_float16[float64](fn)
...
#        return self._cimg + rhs
    ...
    def asarray(self):
        ...
        cdef float64* data = self._cimg.data()
        # For cimg storage format see: http://cimg.eu/reference/group__cimg__storage.html
        cdef float64[:,:,:,::1] mem_view = <float64[:spectrum,:depth,:height,:width]>data
