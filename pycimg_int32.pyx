cdef class CImg_int32:
    cdef CImg[int32] _cimg;

    # Constructors
#    def __cinit__(self, filename):
#        byte_string = filename.encode('UTF-8')
#        cdef char* fn = byte_string
#        self._cimg = CImg[int32](fn)
    ...
    def load_cimg_float16(self, filename):
        byte_string = filename.encode('UTF-8')
        cdef char* fn = byte_string
        self._cimg = from_float16[int32](fn)
...
#        return self._cimg + rhs
    ...
    def asarray(self):
        ...
        cdef int32* data = self._cimg.data()
        # For cimg storage format see: http://cimg.eu/reference/group__cimg__storage.html
        cdef int32[:,:,:,::1] mem_view = <int32[:spectrum,:depth,:height,:width]>data
