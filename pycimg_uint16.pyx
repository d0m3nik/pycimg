cdef class CImg_uint16:
    cdef CImg[uint16] _cimg;

    # Constructors
#    def __cinit__(self, filename):
#        byte_string = filename.encode('UTF-8')
#        cdef char* fn = byte_string
#        self._cimg = CImg[uint16](fn)
    ...
    def load_cimg_float16(self, filename):
        byte_string = filename.encode('UTF-8')
        cdef char* fn = byte_string
        self._cimg = from_float16[uint16](fn)
...
#        return self._cimg + rhs
    ...
    def asarray(self):
        ...
        cdef uint16* data = self._cimg.data()
        # For cimg storage format see: http://cimg.eu/reference/group__cimg__storage.html
        cdef uint16[:,:,:,::1] mem_view = <uint16[:spectrum,:depth,:height,:width]>data
