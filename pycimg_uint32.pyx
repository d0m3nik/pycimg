cdef class CImg_uint32:
    cdef CImg[uint32] _cimg;

    # Constructors
#    def __cinit__(self, filename):
#        byte_string = filename.encode('UTF-8')
#        cdef char* fn = byte_string
#        self._cimg = CImg[uint32](fn)
    ...
    def load_cimg_float16(self, filename):
        byte_string = filename.encode('UTF-8')
        cdef char* fn = byte_string
        self._cimg = from_float16[uint32](fn)
...
#        return self._cimg + rhs
    ...
    def asarray(self):
        ...
        cdef uint32* data = self._cimg.data()
        # For cimg storage format see: http://cimg.eu/reference/group__cimg__storage.html
        cdef uint32[:,:,:,::1] mem_view = <uint32[:spectrum,:depth,:height,:width]>data
