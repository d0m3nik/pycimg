cdef class CImg_uint8:
    cdef CImg[uint8] _cimg;

    # Constructors
#    def __cinit__(self, filename):
#        byte_string = filename.encode('UTF-8')
#        cdef char* fn = byte_string
#        self._cimg = CImg[uint8](fn)
    ...
    def load_cimg_float16(self, filename):
        byte_string = filename.encode('UTF-8')
        cdef char* fn = byte_string
        self._cimg = from_float16[uint8](fn)
...
#        return self._cimg + rhs
    ...
    def asarray(self):
        ...
        cdef uint8* data = self._cimg.data()
        # For cimg storage format see: http://cimg.eu/reference/group__cimg__storage.html
        cdef uint8[:,:,:,::1] mem_view = <uint8[:spectrum,:depth,:height,:width]>data
